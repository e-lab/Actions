...
# setup camera input:
# 0 for default camera input or the path to your input video 
cam = cv2.VideoCapture('/media/HDD1/Datasets/Gesture-eLab-Dataset/pinch/pinch-2.mp4')
cam.set(3, xres)
cam.set(4, yres)
...
while True:
    ...
    output = model(pframe)
    if output is None:
        print('no output from resNet model file')
        break

    output = softMax(output) # convert output to probabilities
    ...
    cv2.putText(frame, text, (10, yres-20), font, 0.5, (255, 0, 0), 1)
