...
class ModelDef(nn.Module):
    ...
    def __init__(self, i_height, i_width, n_classes):
        ...
        self.rnn = nn.RNN(self.n_inp, self.n_classes, 1, nonlinearity='tanh', batch_first=True)

    def forward(self, x, h0):

        yt_seq, hn = self.rnn(x, h0)

        return yt_seq, hn

    def init_hidden(self, bsz):
        weight = next(self.parameters()).data
        return Variable(weight.new(self.n_layers, bsz, self.n_classes).zero_())