...
class ModelDef(nn.Module):
    ...
    def __init__(self, h_n, rnn_type):
        ...
        self.batchnorm = nn.BatchNorm2d(x_n)
...
class ModelBranch(nn.Module):
    ...
    def __init__(self):
        ...
        self.Conv3D1 = BasicConv3D(3, 32, (3,3,3), stride=(3,1,1), pad=(0,1,1), bn = True)
        self.Conv3D2 = BasicConv3D(32, 64, (2,3,3), stride=(1,1,1), pad=(0,1,1), bn = True)

        self.Conv2D1 = BasicConv2D(64, 128, (3,3), stride=(1,1), pad=(1,1), pool=True, bn = True)
        self.Conv2D2 = BasicConv2D(128, 256, (3,3), stride=(1,1), pad=(1,1), pool=True, bn = True)
        self.Conv2D3 = BasicConv2D(256, 256, (3,3), stride=(1,1), pad=(1,1), pool=True, bn = True)
...
class BasicConv3D(nn.Module):
    def __init__(self, iChannels, oChannels, kernel, stride, pad, bn):
        ...
        self.bn = bn
        self.batchnorm = nn.BatchNorm3d(oChannels)
...
class BasicConv2D(nn.Module):
    def __init__(self, iChannels, oChannels, kernel, stride, pad, pool, bn):
        super(BasicConv2D, self).__init__()
        self.Conv2D = nn.Conv2d(iChannels, oChannels, kernel, stride, pad)
        self.bn = bn
        self.batchnorm = nn.BatchNorm2d(oChannels)
        self.pool = pool
        if pool:
