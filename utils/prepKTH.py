...
_('--dim',   type=int, default=(80, 60), nargs=2, help='input image dimension as tuple (WxH)', metavar=('W', 'H'))
            ...
            if (frameCount % (args.skip+1)) == 0:
                ...
                if frameCount > 1:
                    ...
                    for i in range(len(gray_opt)):
                        gray_opt[i] = (gray_opt[i] - gray_opt[i].min())
                                    / (gray_opt[i].max() - gray_opt[i].min())
